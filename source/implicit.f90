...
module implicit
    ...
    real(kind=8), dimension(kx) :: tref  ! Temperature profile for semi-implicit scheme
    real(kind=8), dimension(kx) :: tref1 ! Gas constant * tref
    real(kind=8), dimension(kx) :: tref2 ! akap * tref
    real(kind=8), dimension(kx) :: tref3 ! Full sigma-levels * tref

    real(kind=8), dimension(kx,kx) :: xa, xb, xc, xd, xe
    real(kind=8), dimension(kx,kx,mx+nx+1) :: xf, xj
    real(kind=8) :: dhsx(kx), elz(mx,nx)
...
contains
    ...
    subroutine initialize_implicit(dt)
        ...
        real(kind=8) :: dsum(kx), ya(kx,kx)
        integer :: indx(kx), m, n, k, k1, k2, l
        real(kind=8) :: rgam, xi, xxi, xxx
    ...
    subroutine implicit_terms(divdt,tdt,psdt)
        complex(kind=8), intent(inout) :: divdt(mx,nx,kx) !! Divergence tendency
        complex(kind=8), intent(inout) :: tdt(mx,nx,kx)   !! Temperature tendency
        complex(kind=8), intent(inout) :: psdt(mx,nx)     !! log(surface pressure) tendency

        complex(kind=8) ::  ye(mx,nx,kx), yf(mx,nx,kx)
        integer :: k1, k, m, n
