...
module convection
    ...
    private
    public get_convection_tendencies, diagnose_convection

    ! Constants for convection
    real(kind=8), parameter :: psmin  = 0.8 !! Minimum (normalised) surface pressure for the occurrence
                                       !! of convection
    real(kind=8), parameter :: trcnv  = 6.0 !! Time of relaxation (in hours) towards reference state
    real(kind=8), parameter :: rhbl   = 0.9 !! Relative humidity threshold in the boundary layer
    real(kind=8), parameter :: rhil   = 0.7 !! Relative humidity threshold in intermeduate layers for
                                       !! secondary mass flux
    real(kind=8), parameter :: entmax = 0.5 !! Maximum entrainment as a fraction of cloud-base mass flux
    real(kind=8), parameter :: smf    = 0.8 !! Ratio between secondary and primary mass flux at cloud-base
...
contains
    ...
    subroutine get_convection_tendencies(psa, se, qa, qsat, itop, cbmf, precnv, dfse, dfqa)
        ...
        real(kind=8), intent(in)  :: se(ix,il,kx)   !! Dry static energy [c_p.T + g.z]
        ...
        real(kind=8), intent(out) :: cbmf(ix,il)    !! Cloud-base mass flux
        real(kind=8), intent(out) :: precnv(ix,il)  !! Convective precipitation [g/(m^2 s)]
        real(kind=8), intent(out) :: dfse(ix,il,kx) !! Net flux of dry static energy into each
                                               !! atmospheric layer
        real(kind=8), intent(out) :: dfqa(ix,il,kx) !! Net flux of specific humidity into each
                                               !! atmospheric layer

        integer :: i, j, k, k1, nl1, nlp
        real(kind=8) :: qdif(ix,il)
        real(kind=8) :: entr(2:kx-1), delq, enmass, fdq, fds, fm0, fmass, fpsa, fqmax
        real(kind=8) :: fsq, fuq, fus, qb, qmax, qsatb, rdps, sb, sentr
    ...
    subroutine diagnose_convection(psa, se, qa, qsat, itop, qdif)
        ...
        real(kind=8), intent(in)  :: qsat(ix,il,kx) !! Saturation specific humidity [g/kg]
        integer, intent(out) :: itop(ix,il)    !! Top of convection (layer index)
        real(kind=8), intent(out) :: qdif(ix,il)    !! Excess humidity in convective gridboxes

        integer :: i, j, k, ktop1, ktop2, nl1, nlp
        real(kind=8) :: mss(ix,il,2:kx), mse0, mse1, mss0, mss2, msthr
        real(kind=8) :: qthr0, qthr1, rlhc
        logical :: lqthr
