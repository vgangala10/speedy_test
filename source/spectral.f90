module spectral
    ...
    real(kind=8), dimension(mx,nx) :: el2, elm2, el4, trfilt
    real(kind=8) :: gradx(mx), gradym(mx,nx), gradyp(mx,nx)
    real(kind=8), dimension(mx,nx) :: uvdx, uvdym, uvdyp
    real(kind=8), dimension(mx,nx) :: vddym, vddyp
...
contains
    ...
    function laplacian(input) result(output)
        complex(kind=8), intent(in) :: input(mx,nx)
        complex(kind=8) :: output(mx,nx)
    ...
    function spec_to_grid(vorm, kcos) result(vorg)
        ...
        complex(kind=8), intent(in) :: vorm(mx,nx)
        integer, intent(in) :: kcos

        real(kind=8) :: vorg(ix,il)
        real(kind=8) :: vorm_r(2*mx,nx)
    ...
    function grid_to_spec(vorg) result(vorm)
        ...
        real(kind=8), intent(in) :: vorg(ix,il)
        complex(kind=8) :: vorm(mx,nx)
        real(kind=8) :: vorm_r(2*mx,nx)
    ...
    subroutine grad(psi,psdx,psdy)
        complex(kind=8), dimension(mx,nx), intent(inout) :: psi
        complex(kind=8), dimension(mx,nx), intent(inout) :: psdx, psdy
    ...
    subroutine vds(ucosm,vcosm,vorm,divm)
        complex(kind=8), dimension(mx,nx) :: ucosm, vcosm
        complex(kind=8), dimension(mx,nx), intent(inout) :: vorm, divm
        complex(kind=8), dimension(mx,nx) :: zc, zp
    ...
    subroutine uvspec(vorm,divm,ucosm,vcosm)
        complex(kind=8), dimension(mx,nx), intent(in) :: vorm,divm
        complex(kind=8), dimension(mx,nx), intent(inout) :: ucosm,vcosm
        complex(kind=8), dimension(mx,nx) :: zc,zp
    ...
    subroutine vdspec(ug,vg,vorm,divm,kcos)
        ...
        real(kind=8), intent(in) :: ug(ix,il), vg(ix,il)
        complex(kind=8), intent(out) :: vorm(mx,nx), divm(mx,nx)
        integer, intent(in) :: kcos
        integer :: i, j
        real(kind=8) :: ug1(ix,il), vg1(ix,il)
        complex(kind=8) :: specu(mx,nx), specv(mx,nx)
    ...
    subroutine trunct(vor)
        complex(kind=8), intent(inout) :: vor(mx,nx)
