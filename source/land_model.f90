...
module land_model
    ...
    real(kind=8) :: rhcapl(ix,il) !! 1/heat capacity (land)
    real(kind=8) :: cdland(ix,il) !! 1/dissipation time (land)

    ! Daily observed climatological fields over land
    real(kind=8) :: stlcl_ob(ix,il)   !! Climatological land surface temperature
    real(kind=8) :: snowdcl_ob(ix,il) !! Climatological snow depth (water equivalent)
    real(kind=8) :: soilwcl_ob(ix,il) !! Climatological soil water availability

    ! Land surface fields used by atmospheric model
    real(kind=8) :: stl_am(ix,il)   !! Land surface temperature
    real(kind=8) :: snowd_am(ix,il) !! Snow depth (water equivalent)
    real(kind=8) :: soilw_am(ix,il) !! Soil water availability

    ! Land surface fields from land model
    real(kind=8) :: stl_lm(ix,il) !! Land-model surface temperature

    ! Land masks
    real(kind=8) :: fmask_l(ix,il) !! Fraction of land
    real(kind=8) :: bmask_l(ix,il) !! Binary land mask

    real(kind=8) :: stl12(ix,il,12)   !! Land surface temperature monthly-mean climatology
    real(kind=8) :: snowd12(ix,il,12) !! Snow depth (water equivalent) monthly-mean climatology
    real(kind=8) :: soilw12(ix,il,12) !! Soil water availability monthly-mean climatology

    integer :: land_coupling_flag = 1 !! Flag for land-coupling (0: off, 1: on)

    real(kind=8), parameter :: sd2sc = 60.0 !! Snow depth (mm water) corresponding to snow cover = 1
...
contains
    ...
    subroutine land_model_init
        ...
        ! Auxiliary variables
        integer :: i, j, month
        real(kind=8) :: dmask(ix,il) ! domain mask
        real(kind=8) :: depth_soil, depth_lice, tdland, hcapl, hcapli, flandmin

        ! Soil moisture parameters
        ! Soil wetness at field capacity (volume fraction)
        real(kind=8) :: swcap = 0.30

        ! Soil wetness at wilting point  (volume fraction)
        real(kind=8) :: swwil = 0.17

        ! Threshold for land-sea mask definition (i.e. minimum fraction of
        ! either land or sea)
        real(kind=8) :: thrsh = 0.1

        real(kind=8) :: rsw, sdep1, sdep2, swroot, swwil2
        real(kind=8), dimension(ix,il) :: veg_low, veg_high, veg, swl1, swl2
        integer :: idep2
        ...
        do month = 1, 12
            ...
            call fillsf(stl12(:,:,month), 0.0_8)
        end do

        call forchk(bmask_l, 12, 0.0_8, 400.0_8, 273.0_8, stl12)
        ...
        call forchk(bmask_l, 12, 0.0_8, 20000.0_8, 0.0_8, snowd12)
        ...
        call forchk(bmask_l, 12, 0.0_8, 10.0_8, 0.0_8, soilw12)
    ...
    subroutine couple_land_atm(day)
        ...
        ! ----- WARNING ------- !
        !imont1 = 1 ! FIXME!! - This is a hack used purely for testing purposes so I don't have to import date

        ! Interpolate climatological fields to actual date ! FIXME - SKipping interpolation too
